...
- **当天一直没收到日报？** 三步自查：① 收到看门狗告警邮件了吗（每天 20:30 检查）？里面写明了最可能的原因；② 没收到告警也没收到日报 → 多半电脑一直睡眠，开机后两个任务会自动补跑；③ 着急的话直接双击桌面「每日新闻」手动生成。历史某天的报告错过了可用 `python tools/resend.py --date YYYY-MM-DD` 补发（不重复消耗 AI）。
